...
)
{
    ...
    {
        if (prc_get_padded_wdesc(window, ctx->fwin, &window->wpad) != FN_SUCCESS)
            return FN_FAILURE;
    }
...
)
{
    ...
    winaddr->win = newwin(winaddr->height, winaddr->width, winaddr->y, winaddr->x);
    if (winaddr->win == NULL)
        return FN_FAILURE;
...
)
{
    ...
    if (_prc_get_window_info(window, ctx) != FN_SUCCESS)
        return FN_FAILURE;
